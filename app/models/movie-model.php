...
class movieModel{
    ...
    public function getAll($filter = null, $sort = null , $order = null , $offset = null , $limit = null){
      ...
      if (isset($sort) &&  isset($order) && isset($offset) && isset($limit) && isset($filter)) {
        ...
        $query = $this->db->prepare("SELECT * FROM peliculas WHERE id LIKE '$filter' OR nombre LIKE '$filter' OR descripcion LIKE '$filter' OR estreno LIKE '$filter' OR id_genero_fk LIKE '$filter' ORDER BY $sort  $order LIMIT $offset , $limit ");
        $query->execute();
      ...
      if(!empty($filter) && !empty($sort) && !empty($order)){
        ...
        $query = $this->db->prepare("SELECT * FROM peliculas WHERE id LIKE '$filter' OR nombre LIKE '$filter' OR descripcion LIKE '$filter' OR estreno LIKE '$filter' OR id_genero_fk LIKE '$filter'  ORDER BY $sort  $order ");
        $query->execute();
        return $query->fetchAll(PDO::FETCH_OBJ);
      ...
      if(isset($filter)){
        ...
        $query = $this->db->prepare("SELECT * FROM peliculas WHERE id LIKE '$filter' OR nombre LIKE '$filter' OR descripcion LIKE '$filter' OR estreno LIKE '$filter' OR id_genero_fk LIKE '$filter'  ");
        $query->execute();
